...
#   ./setup_envs.sh simple_monitoring   # set up just one (by requirements filename, without .txt)
#
set -euo pipefail
...
setup_env() {
    ...
    local display_name
    display_name="Fiddler: $(echo "${name}" | tr '_' ' ' | awk '{for(i=1;i<=NF;i++) $i=toupper(substr($i,1,1)) substr($i,2)}1')"
...
echo "  matching Jupyter kernel (e.g. 'Fiddler: Simple Monitoring')."
echo "=============================================="
